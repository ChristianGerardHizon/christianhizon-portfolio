  ...
  FutureEither<PatientTreatmentRecord> create(
    ...
    return TaskEither.tryCatch(
      () async {
        ...
        // Fetch with expand to get treatment relation
        final fetched =
            await _collection.getOne(created.id, expand: 'treatment');
        return PatientTreatmentRecordDto.fromRecord(fetched).toEntity();
      },
      Failure.handle,
    ).run();
  }
    ...
    return TaskEither.tryCatch(
      () async {
        await _collection.update(
          record.id,
          body: PatientTreatmentRecordDto.toUpdateJson(record),
        );
        // Fetch with expand to get treatment relation
        final fetched =
            await _collection.getOne(record.id, expand: 'treatment');
        return PatientTreatmentRecordDto.fromRecord(fetched).toEntity();
      },
      Failure.handle,
    ).run();
