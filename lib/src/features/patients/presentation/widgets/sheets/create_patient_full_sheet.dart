...
/// Enhanced bottom sheet for creating a new patient with full-screen mode support.
///
/// Features:
/// - Simple mode (70% height): Patient info + Owner info
/// - Full-screen mode (95% height): Enables Medical Record + Prescription sections
/// - Progressive disclosure: Sections unlock as required fields are filled
/// - Integrated appointment scheduling
class CreatePatientFullSheet extends HookConsumerWidget {
  const CreatePatientFullSheet({
    super.key,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    useEffect(() {
      void listener() {
        // Consider "full screen" when sheet is at 85%+ of screen
        isFullScreen.value = sheetController.size >= 0.85;
      }
    ...
    void openCreateAppointmentSheet(Patient patient) {
      showModalBottomSheet(
        ...
        builder: (sheetContext) => DraggableScrollableSheet(
          ...
          maxChildSize: 0.95,
          expand: false,
          builder: (_, scrollController) => CreateAppointmentSheet(
            ...
            children: [
              ...
              if (isFullScreen.value) ...[
                ...
                _ActionCard(
                  ...
                  subtitle: 'Save patient first, then schedule a follow-up',
                  enabled: !isSaving.value,
                  onTap: () async {
                    // First save the patient, then open appointment sheet
                    final isValid = formKey.currentState!.saveAndValidate();
                    if (!isValid) {
                      showFormErrorDialog(
                        context,
                    ...
                    // Get the created patient and open appointment sheet
                    if (context.mounted) {
                      final patients =
                          ref.read(patientsControllerProvider).value;
                      final createdPatient = patients?.firstOrNull;
                      if (createdPatient != null) {
                        openCreateAppointmentSheet(createdPatient);
                      }
                    }
                  },
                ),
...
/// Shows the enhanced create patient sheet as a modal bottom sheet.
///
/// This sheet supports simple mode (70% height) and full-screen mode (95% height).
/// In full-screen mode, additional sections for medical records, prescriptions,
/// and appointment scheduling become available.
void showCreatePatientFullSheet(BuildContext context) {
  ...
  showModalBottomSheet(
    ...
    builder: (context) => DraggableScrollableSheet(
      ...
      builder: (context, scrollController) => CreatePatientFullSheet(
